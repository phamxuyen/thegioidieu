...
class ControllerCommonColumnRight extends Controller {
	public function index() {
		...
		if ($route == 'bossblog/blogcategory' && isset($this->request->get['path'])) {
			$this->load->model('bossblog/blogcategory');
			
			$path = explode('_', (string)$this->request->get['path']);

			$layout_id = $this->model_bossblog_blogcategory->getCategoryLayoutId(end($path));
		}
		...
		if ($route == 'bossblog/article' && isset($this->request->get['blog_article_id'])) {
			$this->load->model('bossblog/article');
			
			$layout_id = $this->model_bossblog_article->getArticleLayoutId($this->request->get['blog_article_id']);
		}
